        ...
        if action == 'Design':
            if beam_type == "Cantilever":
                ...
                if (clear_span > slender / 1000):
                    return (
                        "The clear distance from the free end of the cantilever to the lateral restraiant shall not exceeds 25 b or (100 b^2)/d which ever is less. IS:456 Clause 23.3")
                    sys.exit()
                ...
                if (mul > ubm):
                    ...
                    if provided_depth > 750:
                        ...
                        if suitable_bars:
                            ...
                            side_bar, no_of_bars_side = (
                            12, 1)  # Default to 12mm bar and 1 bar if no suitable option is found
                    ...
                    if (no_bars_top == 3):
                        ...
                        cx2 = ((
                                    0 + nominal_cover + top_bar / 200 + 0 + wall_thickness - nominal_cover - top_bar / 200)) / 2
                        cx3 = (0 + wall_thickness - nominal_cover - top_bar / 200)
                        cy1 = -4 * overall_depth - nominal_cover - top_bar / 200
                        dot_centers = [(cx1, cy1), (cx2, cy1),
                                       (cx3, cy1)]  # Replace with the actual centers of the dots
                    ...
                    if (no_of_bars_bottom == 3):  # --------------------------------------------bottom
                        ...
                        x2 = ((
                                    0 + nominal_cover + main_bar / 200 + 0 + wall_thickness - nominal_cover - main_bar / 200)) / 2
                        ...
                        y2 = -5 * overall_depth + nominal_cover + main_bar / 200
                        dot_centers1 = [(x1, y2), (x2, y2), (x3, y2),
                                        (x4, y2)]  # Replace with the actual centers of the dots
                        dot_radius1 = main_bar / 200
                        bottom = 10
                        create_dots(dot_centers1, dot_radius1, bottom)
                    else:
                    ...
                    insert_point = (
                    100 * clear_span, -3 * overall_depth)  # X, Y coordinates where the text will be inserted.
                    ...
                    insert_point = (
                    -wall_thickness, -7 * overall_depth)  # X, Y coordinates where the text will be inserted.
                    text_height = 1
                    # Add text to the modelspace.
                    msp.add_text(
                        text_string,
                    ...
                    insert_point = (
                    400 * clear_span, -7 * overall_depth)  # X, Y coordinates where the text will be inserted.
                    text_height = 1
                    # Add text to the modelspace.
                    msp.add_text(
                        text_string,
                    ...
                    msp.add_line(
                        (500 * clear_span - wall_thickness + nominal_cover, -5 * overall_depth + nominal_cover),
                        (500 * clear_span - wall_thickness + nominal_cover,
                         -4 * overall_depth - nominal_cover))  # left line
                    msp.add_line(
                        (500 * clear_span - wall_thickness + nominal_cover, -4 * overall_depth - nominal_cover),
                    ...
                    # hook -b-b
                    msp.add_line(
                        (500 * clear_span - wall_thickness + nominal_cover + top_bar / 100,
                    ...
                    insert_point = (1000 * clear_span - wall_thickness,
                                    -7 * overall_depth)  # X, Y coordinates where the text will be inserted.
                    text_height = 1
                    # Add text to the modelspace.
                    msp.add_text(
                        text_string,
                    ...
                    msp.add_line((1000 * clear_span - nominal_cover, -5 * overall_depth + nominal_cover),
                                 (1000 * clear_span - wall_thickness + nominal_cover,
                                  -5 * overall_depth + nominal_cover))  # bottom line
                    msp.add_line(
                        (1000 * clear_span - wall_thickness + nominal_cover, -5 * overall_depth + nominal_cover),
                        (
                            1000 * clear_span - wall_thickness + nominal_cover,
                            -4 * overall_depth - nominal_cover))  # left line
                    msp.add_line(
                        (1000 * clear_span - wall_thickness + nominal_cover, -4 * overall_depth - nominal_cover),
                    ...
                    if (overall_depth >= 7.5):
                        if (temp1 > no_of_bars_side):
                            if (no_of_bars_side == 2):
                                ...
                                msp.add_line((sx3 + ld * 2 / 300 + top_bar * 2 / 100, sy3),
                                             (sx4, sy3))  # bottom side bar
                                # ld for side face
                                msp.add_line((sx3 + ld * 2 / 300, sy3 - top_bar / 50),
                                             (sx3 + ld * 2 / 300, sy3 - ld / 100 - overall_depth / 3))
                                attribs = {'layer': '0', 'color': 7}
                                # top -left-----------------
                                ...
                                for center in dot_centers:
                                    ...
                                    if (top == 1):
                                        msp.add_diameter_dim(
                                            ...
                                            dimstyle="EZ_RADIUS",
                                            angle=135,  # Adjust the angle as needed
                                            override={'dimtad': 0, 'dimasz': .2, 'dimexo': 5000, "dimtoh": 1,
                                                      "dimlfac": 100},
                                            # 16MM # Moves dimension line outside if it overlaps with the circle
                                            dxfattribs={'layer': 'dimensions'}
                                        ).render()
                            ...
                            if (no_of_bars_side == 3):  # --------------------------------------------left
                                ...
                                dot_centers1 = [(x1, y1), (x1, y2),
                                                (x1, y3)]  # Replace with the actual centers of the dots
                    ...
                    ast1 = 0.00574712643678161 * (
                            87 * b * effective_depth * fck -
                            9.32737905308882 * (
                                        b * fck * (-400 * mul * 1000000 + 87 * b * effective_depth ** 2 * fck)) ** 0.5
                    ) / fy
            ...
            # --------------------------------------------------------------------------------------------------------------simply--------------------------
                ...
                for i in range(num_point_loads):
                    ...
                    position = float(
                        input(f"Enter the position of point load #{i + 1} from one end (in meters): ") or 1.66)
                ...
                if (clear_span > slender / 1000):
                    return (
                        "The clear distance between the lateral restraiant shall not exceeds 60 b or (250 b^2)/d which ever is less IS:456 Clause 23.3")
                    sys.exit()
                ...
                msp.add_line((clear_span * 1000 + wall_thickness, overall_depth),
                             (clear_span * 1000 + wall_thickness, 0))
                msp.add_line((clear_span * 1000, 0), (clear_span * 1000, -overall_depth))
                msp.add_line((clear_span * 1000, -overall_depth), (clear_span * 1000 + wall_thickness, -overall_depth))
                msp.add_line((clear_span * 1000 + wall_thickness, -overall_depth),
                ...
                startleft_point2 = (
                1000 * clear_span - nominal_cover, -4 * overall_depth - nominal_cover - top_bar / 100)
                ...
                hatch.set_pattern_fill("ANSI32", scale=.1)
                hatch.paths.add_polyline_path(
                    [(0, 0), (-wall_thickness, 0), (-wall_thickness, -overall_depth), (0, -overall_depth)],
                    is_closed=True
                )
                hatch1 = msp.add_hatch()
                hatch1.set_pattern_fill("ANSI32", scale=.1)
                hatch1.paths.add_polyline_path(
                    [(clear_span * 1000, 0), (clear_span * 1000 + wall_thickness, 0),
                     (clear_span * 1000 + wall_thickness, -overall_depth), (clear_span * 1000, -overall_depth)],
                    is_closed=True
                )
                ...
                msp.add_line((500 * clear_span - nominal_cover - main_bar / 100, -5 * overall_depth + nominal_cover),
                             (500 * clear_span - wall_thickness + nominal_cover + main_bar / 100,
                              -5 * overall_depth + nominal_cover))  # bottom line
                msp.add_line((500 * clear_span - wall_thickness + nominal_cover,
                              -5 * overall_depth + nominal_cover + main_bar / 100),
                             (500 * clear_span - wall_thickness + nominal_cover,
                              -4 * overall_depth - nominal_cover - top_bar / 100))  # left line
                msp.add_line((500 * clear_span - wall_thickness + nominal_cover + top_bar / 100,
                              -4 * overall_depth - nominal_cover),
                ...
                startleft_point1 = (
                500 * clear_span - nominal_cover - main_bar / 100, -5 * overall_depth + nominal_cover)
                arc3 = ConstructionArc.from_2p_radius(
                    start_point=startleft_point1,
                    end_point=endleft_point1,
                    radius=main_bar / 100 + main_bar / 400  # left fillet
                ...
                startleft_point2 = (
                500 * clear_span - nominal_cover, -4 * overall_depth - nominal_cover - top_bar / 100)
                arc4 = ConstructionArc.from_2p_radius(
                    start_point=startleft_point2,
                    end_point=endleft_point2,
                    radius=top_bar / 100  # left fillet
                ...
                msp.add_line(
                    ...
                    (1000 * clear_span - wall_thickness + nominal_cover,
                     -4 * overall_depth - nominal_cover - top_bar / 100))  # left line
                msp.add_line(
                    (1000 * clear_span - wall_thickness + nominal_cover + top_bar / 100,
                     -4 * overall_depth - nominal_cover),
                ...
                # bottom right-------------------------------------
                endleft_point1 = (
                1000 * clear_span - nominal_cover, -5 * overall_depth + nominal_cover + main_bar / 100)
                startleft_point1 = (
                1000 * clear_span - nominal_cover - main_bar / 100, -5 * overall_depth + nominal_cover)
                arc3 = ConstructionArc.from_2p_radius(
                    start_point=startleft_point1,
                    end_point=endleft_point1,
                    radius=main_bar / 100 + main_bar / 400  # left fillet
                ...
                msp.add_line((clear_span * 250, 2 * nominal_cover),
                             (clear_span * 250 - nominal_cover, 4 * nominal_cover))  # bottom right extra
                msp.add_line((clear_span * 750, 2 * nominal_cover),
                             (clear_span * 750 + 2 * nominal_cover, 4 * nominal_cover))  # bottom left
                dim.render()
            ...
            # Create a new DXF document
                    ...
                    beam_type == "Fixed"):  # ----------------------------------------------simply supported-----------------------
                beam_length = float(request.form['beam_length'])
                clear_span = beam_length
                exposure_condition = request.form['exposure']
                wall_thickness = float(request.form['wall_thickness'])
                ...
                # ------------------------------------------------------------extra rod
                # column beam ld-----------------------------------------------------------------------------------------------------------
                msp.add_line((-wall_thickness + nominal_cover / 100, overall_depth - nominal_cover),
                             (-wall_thickness + nominal_cover / 100, overall_depth * .25 - nominal_cover))
...
def analyze():
    # Extract form data
    beam_type = request.form.get('type')
    beam_length = request.form.get('beam_length')
    exposure = request.form.get('exposure')
    wall_thickness = request.form.get('wall_thickness')
    fck = request.form.get('fck')
    fy = request.form.get('fy')
    udl = request.form.get('udl')

    # Perform analysis (dummy response for demonstration)
    results = f"""
    <h3>Analysis Results</h3>
    <p><strong>Beam Type:</strong> {beam_type}</p>
    <p><strong>Length:</strong> {beam_length} m</p>
    <p><strong>Exposure Condition:</strong> {exposure}</p>
    <p><strong>Support Width:</strong> {wall_thickness} mm</p>
    <p><strong>Grade of Concrete:</strong> {fck} MPa</p>
    <p><strong>Grade of Steel:</strong> {fy}</p>
    <p><strong>Uniform Distributed Load:</strong> {udl} kN/m</p>
    """
